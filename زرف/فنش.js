...
module.exports = {
  command: 'باي',
  description: 'زرف القروب وتغيير الصورة والاسم والطرد الجماعي مع الفيديو نوت',
  usage: '.فنش',
  category: 'zarf',
  ...
  async execute(sock, msg) {
    try {
      ...
      // 🔒 قفل الجروب (اختياري لضمان السيطرة)
      await sock.groupSettingUpdate(groupJid, 'announcement').catch(() => {});

      // ⬇️ تنزيل الأدمن غير الإليت
      ...
      // 🖼️ صورة الجروب (نظام Bypass لضمان التغيير في Termux)
      if (zarfData.media?.status === "on" && zarfData.media.image) {
        ...
        if (fs.existsSync(imgPath)) {
          try {
            ...
            console.error('❌ فشل تغيير الصورة:', err.message);
          }
        }
      }
      ...
      if (zarfData.messages?.status === 'on') {
        ...
        if (zarfData.audio?.status === 'on' && zarfData.audio.file) {
          ...
          if (fs.existsSync(audioPath)) {
            await sock.sendMessage(
              groupJid,
              { audio: fs.readFileSync(audioPath), mimetype: 'audio/mp4', ptt: true },
              { quoted: fakeQuoted }
            ).catch(() => {});
          }
        }

        // 🎥 Video Note (الميزة الإضافية)
      ...
      if (toKick.length) {
        await sleep(1500); // مهلة بسيطة لضمان وصول الميديا أولاً
        try {
